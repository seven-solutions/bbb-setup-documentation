...
wget $CONFIG_URL/apply_config.sh -O /etc/bigbluebutton/bbb-conf/apply-config.sh
chmod +x /etc/bigbluebutton/bbb-conf/apply-config.sh
sed -e "s|%TURN_SERVER|$TURN_SERVER|g" -i /etc/bigbluebutton/bbb-conf/apply-config.sh
...
SAFE_HOSTS=$(cat /usr/share/bbb-web/WEB-INF/classes/bigbluebutton.properties | grep -v '#' | sed -n '/^bigbluebutton.web.serverURL/{s/.*=//;p}' | sed 's/https\?:\/\///')
...
if [ -f docker-compose.yml ] && [ -d db ]; then
new_pass=$(cat docker-compose.yml | grep -oP 'POSTGRES_PASSWORD=\K.*$');
else
docker run --rm bigbluebutton/greenlight:v2 cat ./docker-compose.yml > docker-compose.yml
new_pass=$(openssl rand -hex 8);
sed -i -e 's/POSTGRES_PASSWORD=password/POSTGRES_PASSWORD='$new_pass'/g' docker-compose.yml
fi;
sed -i -e 's/DB_PASSWORD=password/DB_PASSWORD='$new_pass'/g' .env

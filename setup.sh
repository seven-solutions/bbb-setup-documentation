...
sudo add-apt-repository ppa:ondrej/nginx-mainline  # We need nginx to be compiled with OpenSSL ^1.1.1 to support TLSv1.3
...
certbot --webroot -w /var/www/bigbluebutton-default/ -d $SERVER_URL certonly
crontab -e # 30 2 * * 1 /usr/bin/certbot renew >> /var/log/le-renew.log
openssl dhparam -out /etc/nginx/ssl/dhp-4096.pem 4096

cp bigbluebutton.nginx /etc/nginx/sites-available/bigbluebutton
cp ssl.conf /etc/nginx/ssl/ssl.conf
...
sed -e 's|^bigbluebutton.web.serverURL=http://|bigbluebutton.web.serverURL=https://|g' /usr/share/bbb-web/WEB-INF/classes/bigbluebutton.properties
...
cp bigbluebutton_sip_addr_map.conf /etc/nginx/conf.d/bigbluebutton_sip_addr_map.conf
...
cp external_ipv6.xml /opt/freeswitch/conf/sip_profiles/external_ipv6.xml
...
sed -i 's/POSTGRES_PASSWORD=password/POSTGRES_PASSWORD='$pass'/g'docker-compose.yml
sed -i 's/DB_PASSWORD=password/DB_PASSWORD='$pass'/g' .env
...
sed -e 's|^history=[0-9]\?$|history=1|g' -i /etc/cron.daily/bigbluebutton
sed -e 's|^unrecorded_days=[0-9]\?$|unrecorded_days=1|g' -i /etc/cron.daily/bigbluebutton
...
cp stun-turn-servers.xml /usr/share/bbb-web/WEB-INF/classes/spring/turn-stun-servers.xml
sed -e "s|%TURN_SERVER|$TURN_SERVER|g" -i /etc/nginx/conf.d/bigbluebutton_sip_addr_map.conf
sed -e "s|%TURN_SERVER_SECRET|$TURN_SERVER_SECRET|g" -i /etc/nginx/conf.d/bigbluebutton_sip_addr_map.conf
...
cp metrics.nginx /etc/bigbluebutton/nginx/metrics.nginx
